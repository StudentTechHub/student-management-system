function pointCreator() {
  ...
  point.addEventListener("mouseover", (e) => {
    if (point.label) {
      ...
      point.style.transformOrigin = `${point.getAttribute(
        "cx"
      )}px ${point.getAttribute("cy")}px`;
...
function studentDetails(data) {
  ...
  (data.achievements.length > 5
    ...
    sd.querySelector("[achievements] ul").insertAdjacentElement(
      "afterbegin",
      li
    );
  });
  ...
  keys.forEach((key, index) => {
    ...
    [
      ...
      ["stroke-width", "1.5"],
      ["rx", "10"],
    ].forEach((pair) => rect.setAttribute(pair[0], pair[1]));
    ...
    data.marks[key].forEach((marks, index) => {
      ...
      polyline.setAttribute(
        "points",
        polyline.getAttribute("points") + `${x},${y} `
      );
...
window.addEventListener("load", async (e) => {
  ...
  }); // yaha par rollNo variable mei daal dena, jo bhi roll no. hai uska.
  ...
  const data = {
    ...
    achievements: [
      ...
      "Sample achievement 5",
      "Sample achievement 6",
    ], // the last achievement will be shown first, reason - ek nayi achievement jab append hogi, to latest ko sabse upar dikhana sahi hai aur jo purani hoti jaye vo niche jaati jaye.
    marks: {
      "sample key 1": [
        40, 50, 80, 60, 70,
      ] /** ya fir subject wise, semester wise, sessional wise etc. saaro ka ikatha bhi ho sakta hai, lekin fir usme subject wise include mat karna. */,
