async function script() {
  ...
  let achievedMarks=1249, totalMarks=1425, progressNum=Math.round((achievedMarks/totalMarks)*100); // <--- ise comment mat kariyo warna infinite loop mei fas jayega...isme teri progress aayegi database se fetch hokar
  ...
  const circInterval = setInterval((e) => {
    ...
    alignText();
    document.querySelector('[studentPassed]').innerHTML = `${Math.floor((count / coordinates.length) * totalMarks)} / ${totalMarks}`;
    count += 6;
    if (Math.floor((count / coordinates.length) * 100) >= (progressNum || 100)) {
      text.innerHTML = `${progressNum || 100}%`;
      document.querySelector('[studentPassed]').innerHTML = `${achievedMarks} / ${totalMarks}`;
